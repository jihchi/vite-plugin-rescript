...
- `silent`: If `true`, suppresses the ReScript compiler output in the terminal.
- `skipBuild`: If `true`, the plugin will not start the ReScript build process. Useful if you want to run the ReScript compiler separately,
  e.g. in a different terminal or CI pipeline, or to pass custom arguments to it.
